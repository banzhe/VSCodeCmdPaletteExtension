...
namespace VisualStudioCodeWorkspaceExtension.WorkspacesHelper
{
    public class VSCodeWorkspacesApi
    {
        ...
        private List<VSCodeWorkspace> GetWorkspacesInVscdb(VSCodeInstance vscodeInstance, string filePath)
        {
            ...
            try
            {
                using var sqliteConnection = new SqliteConnection($"Data Source={filePath};Mode=ReadOnly;");
                using var sqlite_cmd = sqliteConnection.CreateCommand();
                ...
                if (!sqlite_datareader.Read() || sqlite_datareader.IsDBNull(0))
                {
                ...
                VSCodeStorageEntries vscodeStorageEntries = JsonSerializer.Deserialize(entries, VSCodeJsonSerializerContext.Default.VSCodeStorageEntries);
            ...
            {
                Debug.WriteLine($"Failed to retrieve workspaces from db: {filePath}. Error: {ex}");
                Console.WriteLine($"Failed to retrieve workspaces from db: {filePath}. Error: {ex}");
            }
